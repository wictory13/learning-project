...
class TodoContent extends React.Component<TodoContentProps, TodoContentState> {
    ...
    render(): JSX.Element {
        ...
        if (this.state.edit) {
            return (
                <EditableFieldContainer onEndEdit={this.handleEndEdit} value={this.props.value} id={this.props.id} />
            );
        } else {
            return (
                <span className={contentStyle} onDoubleClick={this.handleDoubleClick}>
                    {this.props.value}
                    <button className={cn.deleteTodo} onClick={() => this.props.onDeleteTodo(this.props.id)}>
                        ×
                    </button>
                </span>
            );
